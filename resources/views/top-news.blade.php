    ...
    @foreach($breaking as $item)
        <span class="mx-4">
            ...
            <a href="{{ route('detail-news', $item->id) }}"
                class="hover:underline hover:text-red-200 transition-colors ml-2">{{ $item->title }}</a>
        </span>
    @endforeach
@endsection
...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-2 mb-8 scrollbar-none">
            <a href="{{ route('top-news') }}"
                class="cat-tab {{ !request('category') ? 'active' : 'text-gray-600 dark:text-gray-400' }}">🏠 Semua</a>
            @foreach(['Technology', 'Business', 'Lifestyle', 'Sports', 'Health', 'Politics', 'Entertainment', 'Science'] as $cat)
                <a href="{{ route('top-news', ['category' => $cat]) }}"
                    class="cat-tab {{ request('category') == $cat ? 'active' : 'text-gray-600 dark:text-gray-400' }}">{{ $cat }}</a>
            @endforeach
        </div>
        ...
        @if(request('search') || request('category'))
            <div
                class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 text-blue-800 dark:text-blue-300 p-4 rounded-xl mb-6 flex items-center justify-between">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
            ...
            <div class="lg:col-span-1 flex flex-col gap-4">
                ...
                @if(isset($activeAds) && $activeAds->where('placement', 'sidebar')->count() > 0)
                    ...
                    @endphp
                    <div
                        class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden p-3 mb-2">
                        <span
                            class="text-[9px] text-gray-400 uppercase tracking-widest font-bold block mb-2 text-center">Advertisement</span>
                        <a href="{{ route('ads.click', $sidebarAd->id) }}" target="_blank"
                            class="block relative group overflow-hidden rounded-xl">
                            <img src="{{ asset($sidebarAd->image_url) }}" alt="{{ $sidebarAd->title }}"
                    ...
                    <div class="p-2 divide-y divide-gray-50 dark:divide-gray-800">
                        @foreach($trending as $i => $item)
                            <div class="flex items-start gap-3 py-3 px-2">
                                <span
                                    class="popular-num text-2xl shrink-0 leading-none mt-0.5">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                                <a href="{{ route('detail-news', $item->id) }}" class="group">
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest text-red-500 block mb-0.5">{{ $item->category }}</span>
                                    <h4
                                        class="text-sm font-bold text-gray-900 dark:text-white leading-snug group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors line-clamp-2">
                                        {{ $item->title }}
                                    </h4>
                                    <time
                                        class="text-[11px] text-gray-400 mt-1 block">{{ $item->created_at->diffForHumans() }}</time>
                                </a>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="section-title">
            ...
            <a href="{{ route('all-news') }}"
                class="ml-auto text-sm font-bold text-red-600 hover:text-red-700 dark:text-red-400 flex items-center gap-1">
                ...
                @if($index > 0 && $index % 8 === 0)
                    @if(isset($activeAds) && $activeAds->where('placement', 'in_article')->count() > 0)
                        ...
                        <div class="col-span-full">
                            <div
                                class="ad-banner bg-white dark:bg-gray-900 p-2 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                <span
                                    class="text-[9px] text-gray-500 uppercase tracking-widest font-bold block mb-2 text-center">Advertisement</span>
                                <a href="{{ route('ads.click', $inArticleAd->id) }}" target="_blank"
                                    class="block relative group overflow-hidden rounded-xl">
                                    <img src="{{ asset($inArticleAd->image_url) }}"
                                        class="w-full h-[90px] md:h-[120px] object-cover opacity-90 group-hover:opacity-100 transition-opacity"
                    ...
                    @else
                        <div class="col-span-full">
                            <div
                                class="ad-banner bg-gray-100 dark:bg-gray-800 p-2 rounded-2xl border border-gray-200 dark:border-gray-700 flex items-center justify-center h-[90px] md:h-[120px]">
